#!/usr/bin/env zsh
# Project Forest 開発環境への完全自動デプロイスクリプト（zsh対応版）
...
echo "${YELLOW}🔐 ステップ 3: ECRへのログインとイメージプッシュ${NC}"
...
echo "${YELLOW}🎯 ステップ 6: ECSクラスターの確認・作成${NC}"
aws-vault exec shinyat -- aws ecs create-cluster \
  --cluster-name default \
  --region $REGION 2>/dev/null || true

# ステップ 7: VPC情報の取得
echo "${YELLOW}🌐 ステップ 7: VPC情報の取得${NC}"
...
echo "${YELLOW}🚀 ステップ 8: ECSサービスの作成または更新${NC}"
...
if [[ "$TASK_ARN" != "None" ]] && [[ -n "$TASK_ARN" ]]; then
  ...
  if [[ -n "$ENI_ID" ]] && [[ "$ENI_ID" != "null" ]]; then
    ...
    if [[ -n "$PUBLIC_IP" ]] && [[ "$PUBLIC_IP" != "None" ]]; then
      ...
      echo "${GREEN}アプリケーションURL: http://$PUBLIC_IP:3000${NC}"
    else
      echo "${YELLOW}⚠️  パブリックIPの取得中...しばらくお待ちください${NC}"
    fi
  fi
else
  echo "${YELLOW}⚠️  タスクが起動中です。以下のコマンドでログを確認してください：${NC}"
  echo "aws-vault exec shinyat -- aws logs tail /ecs/project-forest-dev --follow"
fi

echo "${GREEN}✅ デプロイスクリプト完了！${NC}"